...
def assert_eq(
    ...
    with allure.step(allure_title):
        try:
            ...
            allure.attach(f'Фактическое значение: {repr(actual_value)}', 'Фактическое значение', allure.attachment_type.TEXT)
            allure.attach(f'Ожидаемое значение: {repr(expected_value)}', 'Ожидаемое значение', allure.attachment_type.TEXT)
            raise


@allure.step('Утверждение истинности: {allure_title}')
def assert_true(condition: bool, allure_title: str, error_msg: Optional[str] = None) -> None:
    with allure.step(allure_title):
...
def assert_json_contains(actual_json: Mapping[str, Any], expected_subset: Mapping[str, Any], allure_title: str) -> None:
    with allure.step(allure_title):
        allure.attach(json.dumps(actual_json, indent=2, ensure_ascii=False), 'Фактический JSON', allure.attachment_type.JSON)
        allure.attach(json.dumps(expected_subset, indent=2, ensure_ascii=False), 'Ожидаемый фрагмент', allure.attachment_type.JSON)
        assert _dict_contains(actual_json, expected_subset), 'JSON не содержит ожидаемые поля/значения'
        allure.attach('JSON содержит ожидаемые поля/значения', 'Результат', allure.attachment_type.TEXT)
...
def validate_response(
    ...
    with allure.step('Детали HTTP запроса'):
        ...
        allure.attach(json.dumps(request_details, indent=2, ensure_ascii=False), 'HTTP запрос', allure.attachment_type.JSON)
    with allure.step('Детали HTTP ответа'):
        ...
        allure.attach(json.dumps(response_details, indent=2, ensure_ascii=False), 'HTTP ответ', allure.attachment_type.JSON)
    ...
    # Подробные проверки содержимого, если ожидается JSON
    if expected_json is not None or expected_json_partial is not None:
        try:
            actual_json = response.json()
            allure.attach(json.dumps(actual_json, indent=2, ensure_ascii=False), 'Фактический JSON ответ', allure.attachment_type.JSON)
        except json.JSONDecodeError:
            allure.attach(response.text, 'Текстовый ответ (не JSON)', allure.attachment_type.TEXT)
            raise AssertionError('Ожидался JSON ответ, но сервер вернул не-JSON')

        if expected_json is not None:
            allure.attach(json.dumps(expected_json, indent=2, ensure_ascii=False), 'Ожидаемый JSON ответ', allure.attachment_type.JSON)
    ...
    with allure.step('Валидация завершена успешно'):
        allure.attach('Все проверки пройдены успешно', 'Результат валидации', allure.attachment_type.TEXT)


# Helper functions for common test patterns
@allure.step('Валидация структуры JSON ответа')
def validate_json_structure(response_data: dict, required_fields: List[str], allure_title: str = 'Проверка структуры JSON') -> None:
    """Проверяет наличие обязательных полей в JSON ответе"""
    with allure.step(allure_title):
        missing_fields = [field for field in required_fields if field not in response_data]
        if missing_fields:
            allure.attach(
                json.dumps({'missing_fields': missing_fields, 'available_fields': list(response_data.keys())}, indent=2),
                'Отсутствующие поля',
                allure.attachment_type.JSON
            )
            raise AssertionError(f'Отсутствуют обязательные поля: {missing_fields}')
        allure.attach('Все обязательные поля присутствуют', 'Результат', allure.attachment_type.TEXT)


@allure.step('Валидация ответа каталога')
def validate_catalog_response(response_data: List[dict], min_items: int = 0) -> None:
    """Проверяет структуру ответа каталога товаров"""
    with allure.step('Проверка каталога товаров'):
        ...
        allure.attach('Каталог товаров валиден', 'Результат', allure.attachment_type.TEXT)


@allure.step('Валидация ответа корзины')
def validate_cart_response(response_data: dict) -> None:
    """Проверяет структуру ответа корзины"""
...
def validate_order_response(response_data: dict, expected_order_id: Optional[int] = None) -> None:
    """Проверяет структуру ответа заказа"""
    with allure.step('Проверка заказа'):
        ...
        # Проверяем ID заказа если передан
        ...
        # Проверяем структуру товаров в заказе
        if response_data['items']:
            item = response_data['items'][0]
