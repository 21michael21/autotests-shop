...
def test_sql_injection_username(shop_service):
    ...
    for username in malicious_usernames:
        ...
        assert resp.status_code in [400, 500], f"SQL инъекция {username} должна быть отклонена"
...
def test_special_characters_username(shop_service):
    ...
    for username in special_chars:
        resp = adapter.register_user(username, "ValidPass123!")
        assert resp.status_code in [400, 500], f"Username с спецсимволом {repr(username)} должен быть отклонен"
