...
def db_client() -> DbClient:
    with allure.step(
        f"Создание DB клиента для {DB_CONFIG['host']}:{DB_CONFIG['port']}"
    ):
...
def auth_adapter(http_client: HTTPClient) -> AuthAdapter:
    with allure.step("Создание AuthAdapter"):
        ...
        allure.attach(
            "AuthAdapter успешно создан", "Статус создания", allure.attachment_type.TEXT
        )
...
def cart_adapter(http_client: HTTPClient) -> CartAdapter:
    with allure.step("Создание CartAdapter"):
        ...
        allure.attach(
            "CartAdapter успешно создан", "Статус создания", allure.attachment_type.TEXT
        )
...
@pytest.fixture
def add_random_item(
    cart_adapter: CartAdapter, random_item: dict, user: dict, db_client: DbClient
) -> dict:
