...
pytestmark = [allure.epic('Система корзины'), allure.feature('Операции с корзиной'), allure.story('Добавление и удаление товаров')]

@allure.title('Добавление товара в корзину')
@allure.severity(allure.severity_level.CRITICAL)
def test_add_item_to_cart(cart_adapter, add_random_item, user):
    with allure.step('Добавление товара в корзину'):
        resp = cart_adapter.add_item(user['token'], add_random_item['item_id'], add_random_item['quantity'])
        allure.attach(str(resp.json()), 'Ответ API', allure.attachment_type.JSON)
    validate_response(resp, http.HTTPStatus.OK)
    
    response_data = resp.json()
    assert 'message' in response_data
    assert response_data['message'] in ['Item added to cart', 'Товар добавлен в корзину']
...
def test_remove_item_from_cart(cart_adapter, add_random_item, user):
    ...
    response_data = resp.json()
    assert 'message' in response_data
    assert response_data['message'] in ['Item removed from cart', 'Товар удалён из корзины', 'Товар удален из корзины']
