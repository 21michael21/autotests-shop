    ...
    def __init__(self, catalog_adapter: CatalogAdapter, token: str) -> None:
        ...
        with allure.step("Инициализация ItemBuilder"):
            initialization_details = {
                "catalog_adapter": str(catalog_adapter),
                "token_type": "JWT",
                "token_length": len(token),
            }
            allure.attach(
                json.dumps(initialization_details, indent=2),
                "Конфигурация ItemBuilder",
                allure.attachment_type.JSON,
            )
            allure.attach(
                "ItemBuilder готов к работе с каталогом",
        ...
        with allure.step("Получение случайного товара из каталога"):
            ...
            with allure.step("Проверка наличия товаров"):
                if not items:
                    ...
                    raise ValueError(error_msg)
                allure.attach(
                    f"В каталоге найдено {len(items)} товаров",
                    "Результат проверки",
                    allure.attachment_type.TEXT,
                )
            with allure.step("Выбор случайного товара"):
                ...
                allure.attach(
                    f"Выбран товар: {selected_item.get('name', 'Unknown')} (ID: {selected_item.get('id', 'Unknown')})",
            ...
            with allure.step("Генерация количества товара"):
                ...
                allure.attach(
                    f"Количество товара: {quantity} (диапазон: 1-5)",
                    "Детали генерации количества",
                    allure.attachment_type.TEXT,
                )
            with allure.step("Формирование итоговых данных"):
                ...
                allure.attach(
                    ...
                    "Итоговые данные товара",
                    allure.attachment_type.JSON,
                )
                validation_result = {
                    "item_id": {
                        "value": item_data["item_id"],
                        "type": type(item_data["item_id"]).__name__,
                        "is_valid": isinstance(item_data["item_id"], int),
                    },
                    "quantity": {
                        "value": item_data["quantity"],
                        "type": type(item_data["quantity"]).__name__,
                        "is_valid": isinstance(item_data["quantity"], int)
                        and 1 <= item_data["quantity"] <= 5,
                    },
                }
                allure.attach(
                ...
                allure.attach(
                    "Тестовые данные товара успешно сформированы",
